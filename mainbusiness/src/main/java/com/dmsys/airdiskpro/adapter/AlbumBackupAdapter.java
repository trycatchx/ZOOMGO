...
public class AlbumBackupAdapter extends BaseAdapter {
	...
	public AlbumBackupAdapter(Context mContext,
			List<MediaFolder> mGroupDatas, int imageRLWIdth) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.album_backup_item, null);
		...
		loadData2Holder(mGroupDatas.get(position), mDMImageLoader,
	...
	private class PicHolder {
		...
		public LinearLayout backup_choose_icon;
	...
	private void initHolder(PicHolder holder, View convertView) {
		...
		holder.backup_choose_icon = (LinearLayout) convertView
				.findViewById(R.id.backup_choose_icon);
	...
	private void loadData2Holder(MediaFolder folder, DMImageLoader imageLoader,
		...
		loadOperatorDate(holder.backup_choose_icon, folder.isSelected());
		loadFolderInfo(holder, folder.getFolderName(), folder
				.getMediaInfoList().size(), width);
	...
	private void loadOperatorDate(LinearLayout operView, boolean selected) {
	...
	private void loadVideo(ImageView iconview, MediaInfo info) {
		iconview.setScaleType(ScaleType.CENTER_CROP);
		DMFile item = new DMFile();
		item.mPath = info.getPath();
		item.mLocation = DMFile.LOCATION_LOCAL;
		item.mType = DMFileCategoryType.E_VIDEO_CATEGORY;
		...
		if (null == b) {
			int defaultIconId = -1;
			defaultIconId = R.drawable.bt_download_manager_video_big;
	...
	private void loadMedia(DMImageLoader imageLoader,
			DisplayImageOptions options, final ImageView iconView, MediaInfo info) {
		
		if(info != null && info.mMediaInfoType == Type.VIDEO) {
			loadVideo(iconView, info);
			//取消之前的引用下载	
			//iconView.setImageResource(R.drawable.bt_download_manager_video_big);
