...
public class DropBoxAttributeTask extends AsyncTask<String, String, Boolean>
	...
	public DropBoxAttributeTask(Context mContext) {
		this.mContext = mContext;
		taskFlag = true;
	}

	public DropBoxAttributeTask(Context mContext, DMFile mfiledata) {
		this(mContext);
		this.mfiledata = mfiledata;
	}

	public DropBoxAttributeTask(Context mContext, boolean typeFolder,
			DMFile mfiledata) {
		this(mContext);
	...
	public DropBoxAttributeTask(Context mContext, List<DMFile> mlfd) {
		this(mContext);
	...
	public boolean showAttributeDialog(DMFile filedata) {
		...
		Log.e("DialogLogic - showAttributeDialog", "absolutePath:"
				+ absolutePath);
		Log.e("DialogLogic - showAttributeDialog", "fileDate:" + fileDate
				+ "\t sfileDate:" + sfileDate);

		if (filedata.isDir()) {
			if (filedata.mLocation == DMFile.LOCATION_DROPBOX) {
	...
	private boolean getFileLocalInfo(File mFile) {
		...
		if (mFile.isDirectory()) {// 文件夹
			folderNum += 1;
			List<DMFile> arrfile = FileOperationHelper.getInstance()
					.listLocalFolderAllFiles(mFile.getAbsolutePath(), false);
			if (arrfile != null) {
	...
	private boolean getCurLevelFileLocalInfo(File mFile,
			DMFileCategoryType mType) {
		if ((mFile == null) || (taskFlag == false)) {
			return false;
		}
	...
	public boolean WebDirAttribute(DMFile file) {
		...
		if (!typeFolder) {
			...
			try {
				fileList = DMDropboxAPI.getInstance().listFileDropBox(
						file.mPath, -1, null, true, null);
			} catch (Exception e) {
	...
	public boolean WebCurLevelDirAttribute(DMFile file, DMFileCategoryType mType) {
		...
		List<DMFile> fileList = null;
		try {
			fileList = FileOperationHelper.getInstance()
					.getUdiskFolderAllFiles(file);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void updateMsg() {
		...
		switch (showType) {
		case SHOWTYPEISFILE: {
			String sizeToShow = String.format(
					mContext.getString(R.string.DM_More_Detail_Size), sSize);
		...
		case SHOWTYPEISDIR: {
			...
			if (typeFolder) {
				containToShow = String.format(mContext
						.getString(R.string.DM_More_Detail_Contain_Folder),
						folderNum);
			} else {
				containToShow = String.format(
						mContext.getString(R.string.DM_More_Detail_Contain),
						folderNum, fileNum);
			}
		...
		case SHOWTYPEISLIST: {
			String sizeToShow = String.format(
					mContext.getString(R.string.DM_More_Detail_Size), sSize);
			String containToShow = String.format(
					mContext.getString(R.string.DM_More_Detail_Contain),
					folderNum, fileNum);
	...
	public void showAlertDislog() {
		...
		attributeDialog = new UDiskAttributeDialog(mContext,
				UDiskBaseDialog.TYPE_ONE_BTN);
		attributeDialog.setTitleContent(mContext.getResources().getString(
				R.string.DM_Task_Details));
		if (showType == SHOWTYPEISLIST) {
			attributeDialog.getTypeImageView().setImageResource(
					R.drawable.file_type_multi);
			attributeDialog.getNameTextView().setText(
					R.string.DM_More_Detail_Multiple);
			attributeDialog
					.getTypeTextView()
					.setText(
							String.format(
									mContext.getString(R.string.DM_More_Detail_Species),
									mContext.getString(R.string.DM_More_Detail_Other)));
			...
			attributeDialog
					...
					.setText(
							String.format(
									mContext.getString(R.string.DM_More_Detail_Species),
									FileUtil.getFileTypeString(mfiledata)));
			attributeDialog.getTypeImageView().setImageResource(
					FileUtil.getFileLogo(mfiledata));
			attributeDialog.getLastModifyTextView().setText(
					mfiledata.getLastModified("yyyy-MM-dd HH:mm:ss"));
			attributeDialog.getNameTextView().setText(mfiledata.getName());
			attributeDialog.getPathTextView().setText(mfiledata.getPath());

			if (mfiledata.isDir())
				attributeDialog.getContainRelativeLayout().setVisibility(
						View.VISIBLE);
			else
				attributeDialog.getContainRelativeLayout().setVisibility(
						View.GONE);
		...
		updateMsg();
		attributeDialog.setLeftBtn(
				mContext.getString(R.string.DM_SetUI_Confirm), null);
