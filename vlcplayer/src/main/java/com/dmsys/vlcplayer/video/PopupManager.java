...
package com.dmsys.vlcplayer.video;
...
import com.dmsys.vlcplayer.view.PopupLayout;
...
public class PopupManager implements PlaybackService.Callback, GestureDetector.OnDoubleTapListener,
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.video_play_pause){
        ...
        }else if (v.getId() == R.id.popup_close){
            stopPlayback();
        }else if (v.getId() == R.id.popup_expand){
            mService.removePopup();
            mService.switchToVideo();
        }
    }
    ...
    private void stopPlayback() {
        ...
        if (mService.isSeekable() && time != -1)
            editor.putLong(Preferences.VIDEO_RESUME_TIME, time).apply();
    }
